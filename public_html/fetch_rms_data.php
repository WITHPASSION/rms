...
function fetch_req_call_data($year_month, $year, $month, $bill_payer_id) {
	...
	if (!empty($shakkin) ||
		...
		$call_sum = $shakkin+$souzoku+$koutsujiko+$ninibaikyaku+$meigihenkou+$setsuritsu+$keijijiken+$rikon+$bgatakanen;
		$stmt = $pdo2->prepare("
			REPLACE INTO
				monthly_valid_call
			VALUES(
...
function fetch_req_mail_data($year_month, $year, $month, $bill_payer_id) {
	...
	if (!empty($m_shakkin) ||
		...
		$mail_sum =	$m_shakkin+$m_souzoku+$m_koutsujiko+$m_ninibaikyaku+$m_meigihenkou+$m_setsuritsu+$m_rikon+$m_bgatakanen;
		$stmt = $pdo2->prepare("
			REPLACE INTO
				monthly_mail_num
			VALUES(
