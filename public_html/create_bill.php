...
function create_monthly_details($year,$month,$year_month){
	...
	#出力時の行を定義
	$i = 3;
	$stmt = $pdo_request->query("SELECT req_id,req_ad_name FROM ad_req_data");
...
$arr_media_type_data = $stmt->fetchAll(PDO::FETCH_ASSOC);
	foreach ($arr_req_ad_data as $row) {
		...
		foreach ($arr_ad_id as $row) {
			...
			foreach ($arr_ad_name as $row ){
				$ad_name = $row['office_name'];
				$stmt = $pdo_cdr->query("SELECT * FROM call_data_view WHERE DATE_FORMAT(date_from,'%Y%m')=$year_month AND advertiser_id = $ad_id ORDER BY date_from");
...
#出力時の行を定義
$i = 3;
$stmt = $pdo_cdr->query("SELECT * FROM mail_conv WHERE DATE_FORMAT(register_dt,'%Y%m') = $year_month ORDER BY register_dt");
...
foreach ($arr_detail_mail_data as $row) {
	...
	$stmt = $pdo_wordpress->query("SELECT * FROM ss_advertisers WHERE ID = $advertiser_id");
